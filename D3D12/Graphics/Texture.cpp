...
void TextureCube::Create(Graphics* pGraphics, CommandContext* pContext, const char* pFilePath, TextureUsage usage)
{
	Image img;
	if (img.Load(pFilePath))
	{
		...
		for (int i = 0; i < 6; ++i)
		{
			...
			for (int j = 0; j < m_MipLevels; ++j)
			{
				...
				MipLevelInfo info = pCurrentImage->GetMipInfo(j);
