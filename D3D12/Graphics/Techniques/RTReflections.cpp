	...
	rt.Bind([=](CommandContext& context, const RGPassResources& passResources)
		{
			...
			context.DispatchRays(bindingTable, m_pTestOutput->GetWidth(), m_pTestOutput->GetHeight());
		});

	ImGui::Image(m_pTestOutput.get(), ImVec2(1024, 1024));
}

void RTReflections::SetupResources(Graphics* pGraphics)
{
	m_pTestOutput = std::make_unique<Texture>(pGraphics);
	m_pTestOutput->Create(TextureDesc::Create2D(512, 512, DXGI_FORMAT_R8G8B8A8_UNORM, TextureFlag::UnorderedAccess | TextureFlag::ShaderResource, 1, 1));
}
