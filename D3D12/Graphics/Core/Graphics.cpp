...
void Graphics::Update()
{
	...
	if (shadowIndex > m_ShadowMaps.size())
	{
		...
		for (auto& pShadowMap : m_ShadowMaps)
		{
			...
			++i;
			RegisterBindlessTexture(pShadowMap.get(), nullptr);
		}
	}
...
void Graphics::InitializeAssets(CommandContext& context)
{
	...
	if (SupportsRayTracing())
	{
		...
		{
			uint32 numMeshes = (uint32)m_Meshes.size();
			...
			prebuildInfo.NumDescs = numMeshes;
			...
			for (uint32 i = 0; i < numMeshes; ++i)
			{
				...
				pInstanceDesc->InstanceContributionToHitGroupIndex = meshCount;
				pInstanceDesc->InstanceID = i;
			...
			asDesc.Inputs.NumDescs = numMeshes;
...
int Graphics::RegisterBindlessTexture(Texture* pTexture, Texture* pFallback)
{
	...
	return pFallback ? RegisterBindlessTexture(pFallback) : 0;
}
