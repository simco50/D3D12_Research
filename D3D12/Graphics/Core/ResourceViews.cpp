...
void ShaderResourceView::Create(Buffer* pBuffer, const BufferSRVDesc& desc)
{
	...
	m_GpuDescriptor = m_pParent->GetParent()->StoreViewDescriptor(m_Descriptor);
}
...
void ShaderResourceView::Release()
{
	if (m_Descriptor.ptr != 0)
	{
		...
		m_Descriptor.ptr = 0;
		m_pParent->GetParent()->FreeViewDescriptor(m_GpuDescriptor);
	}
}
...
void UnorderedAccessView::Create(Buffer* pBuffer, const BufferUAVDesc& desc)
{
	...
	if (desc.Counter)
	{
		...
		m_pCounter = GetParent()->GetParent()->CreateBuffer(BufferDesc::CreateByteAddress(4), name.c_str());
	}
