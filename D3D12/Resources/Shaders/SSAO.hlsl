...
void CSMain(CS_INPUT input)
{
    ...
    float3 viewPos = NormalizedScreenToView(float4(texCoord.xy, depth, 1), cProjectionInverse).xyz;
    float2 texCoord1 = texCoord + float2(dimInv.x, 0);
    float2 texCoord2 = texCoord + float2(0, -dimInv.y);
	float3 p1 = NormalizedScreenToView(float4(texCoord1, tDepthTexture.SampleLevel(sSampler, texCoord1, 0).r, 1), cProjectionInverse).xyz;
	float3 p2 = NormalizedScreenToView(float4(texCoord2, tDepthTexture.SampleLevel(sSampler, texCoord2, 0).r, 1), cProjectionInverse).xyz;
