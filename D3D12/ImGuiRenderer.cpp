	...
	: m_pDevice(pDevice)
{
	...
	std::ifstream file("Resources/imgui.hlsl", std::ios::ate);
	if (file.fail())
	{
		return;
	}
	...
	ComPtr<ID3DBlob> pErrorBlob, pVertexShader, pPixelShader;
	...
	D3DCompile2(data.data(), data.size(), nullptr, nullptr, nullptr, "PSMain", "ps_5_0", compileFlags, 0, 0, nullptr, 0, pPixelShader.GetAddressOf(), pErrorBlob.GetAddressOf());
	if (pErrorBlob != nullptr)
	{
		std::wstring errorMsg = std::wstring((char*)pErrorBlob->GetBufferPointer(), (char*)pErrorBlob->GetBufferPointer() + pErrorBlob->GetBufferSize());
		std::wcout << errorMsg << std::endl;
	...
	HR(pDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(m_pPipelineState.GetAddressOf())));
...
void ImGuiRenderer::Render(CommandContext& context)
{
	...
	Matrix projectionMatrix = XMMatrixOrthographicOffCenterLH(0.0f, (float)1240, (float)720, 0.0f, 0.0f, 1.0f);
	context.SetDynamicConstantBufferView(0, &projectionMatrix, sizeof(Matrix));
	context.SetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	context.SetViewport(Rect{ 0, 0, 1240, 720 }, 0, 1);
	...
	for (int n = 0; n < pDrawData->CmdListsCount; n++)
	{
		...
		for (int cmd_i = 0; cmd_i < pCmdList->CmdBuffer.Size; cmd_i++)
		{
			...
			{
				//context.SetScissorRect({(int)pcmd->ClipRect.x, (int)pcmd->ClipRect.y, (int)pcmd->ClipRect.z, (int)pcmd->ClipRect.w });
